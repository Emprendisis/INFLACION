...
def load_data() -> pd.DataFrame:
    # Streamlit Cloud: usa Secrets (Manage app -> Settings -> Secrets)
    # Debe existir: BMX_TOKEN="..."
    token = st.secrets.get("BMX_TOKEN", None)
    if not token:
        raise RuntimeError(
            "Falta BMX_TOKEN. Configura el token en Streamlit Cloud: "
            "Manage app -> Settings -> Secrets (BMX_TOKEN=...)"
        )
...
with col2:
    ...
    st.metric("Inflación anual (%)", f"{last['dato']:.2f}")
