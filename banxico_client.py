...
class BanxicoClient:
    def __init__(self, token: str | None = None, timeout: int = 30):
        ...
        if not self.token:
            raise ValueError(
                "Falta el token. Define BMX_TOKEN en variables de entorno o pásalo al constructor."
            )
    ...
    def get_series(self, serie: str, start_date: str | None = None, end_date: str | None = None) -> dict:
        ...
        # En Streamlit Cloud el mensaje de HTTPError se redacta; esto deja el motivo visible.
        if r.status_code != 200:
            raise RuntimeError(f"Banxico HTTP {r.status_code}: {r.text[:500]}")
